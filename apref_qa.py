...
old_sol = None
for f in glob.glob(old + '/apref' + old + '.snx'):
    old_sol = f
if old_sol is None:
    raise TypeError
old_dis = None
for f in glob.glob(old + '/disconts' + old + '.snx'):
    old_dis = f
if old_dis is None:
    raise TypeError
new_sol = None
for f in glob.glob(new + '/apref' + new + '.snx'):
    new_sol = f
if new_sol is None:
    raise TypeError
new_dis = None
for f in glob.glob(new + '/disconts' + new + '.snx'):
    new_dis = f
if new_dis is None:
    raise TypeError
...
fout = open('apref' + new + '.txt', 'w')
fout.write('1.0 QA report for APREF(' + new + ')\n\n')
...
fout.write('2.0 Comparison of APREF(' + new + ') with APREF(' + old)
fout.write(')\n\n')
...
# Check for stations that have moved by more than 1mm in any one
# direction (enu)
data_old = {}
for estimate in estimates_old:
...
for estimate in estimates_new:
    try:
        ...
        if abs(e) > 0.001 or abs(n) > 0.001 or abs(u) > 0.001:
            if first:
                fout.write('2.6 The following solutions moved by more than ')
                fout.write('1mm:\n')
                first = False
            line = 'Site: {} Solution: {} Shift (enu): {:.1f} {:.1f} {:.1f}\n'\
                .format(estimate[0], estimate[1], e*1000, n*1000, u*1000)
            fout.write(line)
    except KeyError:
        pass
if first:
    fout.write('2.6 No solutions moved by more than 1mm\n')
fout.write('\n')

# Check for positional uncertainties that have changed by +/-10%
vcv_old = {}
for matrix in matrices_old:
    vcv = array([[matrix[2], matrix[3], matrix[4]],
                   [matrix[3], matrix[5], matrix[6]],
...
for estimate in estimates_old:
    try:
        (lat, lon, h) = xyz2llh(estimate[3], estimate[4], estimate[5])
        vcv_local = vcv_cart2local(vcv_old[estimate[0] + estimate[1]],
                                   lat, lon)
